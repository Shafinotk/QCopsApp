...
def process_csv(file_bytes: bytes) -> bytes:
    ...
    Synchronous and prints progress (so wrapper/streamlit can show logs).
    """
    # Detect encoding
    ...
    first_col = _find_column(df, "First Name") or _find_column(df, "First") or "First Name"
    last_col = _find_column(df, "Last Name") or _find_column(df, "Last") or "Last Name"
    company_col = _find_column(df, "Company Name") or _find_column(df, "Company") or "Company Name"
    title_col = _find_column(df, "Title") or "Title"
    domain_col = _find_column(df, "Domain") or "Domain"

    # Ensure columns exist
    for col in [first_col, last_col, company_col, title_col, domain_col]:
    ...
    total = len(df)
    found_count = 0
    print(f"[file_handler] Processing {total} rows (using columns: {first_col}, {last_col}, {company_col}, {title_col}, {domain_col})")
    ...
    for idx, row in df.iterrows():
        try:
            ...
            link = find_link(first, last, company, title, domain)
            if link:
    ...
    # Write CSV back to bytes (use utf-8 for compatibility)
